//! FLICK_LADDER: commanded absolute yaw flicks, 60s click scoring.
...
pub const FLICK_LADDER_TRIAL_TYPE: &str = "FLICK_LADDER";
pub const FLICK_LADDER_TASK_VERSION: &str = "1";
pub const FLICK_LADDER_DURATION_SECS: f64 = 60.0;

/// Absolute yaw offsets (degrees) from world forward (−Z).
pub const FLICK_LADDER_YAW_SET: [f64; 6] = [-90.0, -60.0, -45.0, 45.0, 60.0, 90.0];

pub fn flick_ladder_task_config_json() -> String {
    format!(
        r#"{{"duration_secs":{},"radius":{},"aim_distance":{},"yaw_set":[-90,-60,-45,45,60,90],"pitch_deg":0,"scoring_rule":"click","rng":"lcg","rng_version":"1"}}"#,
        FLICK_LADDER_DURATION_SECS, AIM_TARGET_RADIUS, AIM_DISTANCE,
    )
}

pub fn flick_ladder_center(yaw_deg: f64) -> Vec3 {
...
pub fn next_commanded_yaw(rng: &mut u64, prev: Option<f64>) -> f64 {
    let mut choices: Vec<f64> = FLICK_LADDER_YAW_SET.to_vec();
    if let Some(p) = prev {
        choices.retain(|y| (*y - p).abs() > 1e-9);
    }
    if choices.is_empty() {
        choices = FLICK_LADDER_YAW_SET.to_vec();
    }
...
fn spawn_commanded(trial: &mut AimTrial, yaw_deg: f64, timestamp_ns: u64) {
    ...
    trial.current_center = flick_ladder_center(yaw_deg);
    trial.last_yaw_deg = yaw_deg;
    push_spawn(trial, yaw_deg, timestamp_ns);
}

pub fn start_flick_ladder_trial(
    pose: &mut YawPitch,
    trial: &mut AimTrial,
    ...
    trial.task_kind = AimTaskKind::FlickLadder;
...
pub fn flick_ladder_should_end(trial: &AimTrial, now_ns: u64) -> bool {
    active_elapsed_ns(trial, now_ns) as f64 / 1e9 >= FLICK_LADDER_DURATION_SECS
}

pub fn finish_flick_ladder_trial(trial: &mut AimTrial, end_ns: u64) -> bool {
    if trial.phase != AimPhase::Armed || trial.task_kind != AimTaskKind::FlickLadder {
        return false;
    }
    let elapsed = active_elapsed_ns(trial, end_ns) as f64 / 1e9;
    trial.score_secs = Some(elapsed.min(FLICK_LADDER_DURATION_SECS));
...
pub fn apply_flick_ladder_shot(trial: &mut AimTrial, pose: &YawPitch, timestamp_ns: u64) -> bool {
    if trial.phase != AimPhase::Armed || trial.task_kind != AimTaskKind::FlickLadder {
        return false;
    }
...
mod tests {
    ...
    #[test]
    fn yaw_set_has_six_absolute_offsets() {
        assert_eq!(FLICK_LADDER_YAW_SET.len(), 6);
        assert!(FLICK_LADDER_YAW_SET.contains(&90.0));
        assert!(FLICK_LADDER_YAW_SET.contains(&-90.0));
    }
    ...
    fn next_yaw_avoids_immediate_repeat() {
        ...
        for _ in 0..40 {
            ...
            assert!(FLICK_LADDER_YAW_SET.iter().any(|v| (*v - y).abs() < 1e-9));
            if let Some(p) = prev {
                assert!((y - p).abs() > 1e-9);
            }
            prev = Some(y);
    ...
    fn finishes_at_sixty_active_seconds() {
        ...
        assert!(start_flick_ladder_trial(
            &mut pose,
            &mut trial,
        ...
        assert!(!flick_ladder_should_end(&trial, start + 59_999_999_999));
        assert!(flick_ladder_should_end(&trial, start + 60_000_000_000));
        assert!(finish_flick_ladder_trial(&mut trial, start + 60_000_000_000));
        let record = build_completed_aim_trial_record(&trial, 2, start + 60_000_000_000);
        assert_eq!(record.trial_type, FLICK_LADDER_TRIAL_TYPE);
        assert_eq!(record.task_version, FLICK_LADDER_TASK_VERSION);
